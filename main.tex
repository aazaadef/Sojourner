...
Results show that MCQ scores improved from 8.30 to 9.15/11 ($r = 0.336$, medium effect), with low-baseline students benefiting most (normalized gain = 0.50).
Self-confidence in analyzing test failures increased ($+0.40$, 7~improved, 0~declined), 95\% of students agreed the activity helped them understand testing vs.\ debugging, and 86\% found it engaging.
...
Table~\ref{tab:mcq} summarizes the MCQ results ($N = 20$ matched pairs, 22 pre, 21 post).
The mean score increased from 8.30 to 9.15 out of 11 ($\Delta = +0.85$).
The Wilcoxon test showed a positive trend ($Z = 1.503$, $p = 0.104$, $r = 0.336$, medium effect).
...
\begin{table}[htbp]
\caption{MCQ Scores: Pre vs.\ Post ($N = 20$)}
\label{tab:mcq}
\centering
\begin{tabular}{lccc}
\toprule
...
\midrule
Mean & 8.30 & 9.15 & +0.85 \\
Improved / Same / Declined & \multicolumn{3}{c}{9 / 7 / 4} \\
Wilcoxon $Z$ ($p$) & \multicolumn{3}{c}{1.503 (0.104)} \\
Effect size $r$ & \multicolumn{3}{c}{0.336 (medium)} \\
Normalized gain & \multicolumn{3}{c}{0.242} \\
\bottomrule
\end{tabular}
\end{table}

Sub-group analysis (Table~\ref{tab:subgroup}) reveals that low-baseline students ($< 8$) gained substantially (normalized gain = 0.50), while high-baseline students ($\geq 10$) showed no change---confirming the ceiling effect and suggesting an equity benefit from the game's scaffolding.

\begin{table}[htbp]
\caption{MCQ Sub-Group Analysis by Baseline}
\label{tab:subgroup}
\centering
\begin{tabular}{lcccc}
\toprule
...
Mid (8--9) & 14 & 8.43 & 8.93 & 0.238 \\
High ($\geq 10$) & 3 & 10.33 & 10.33 & 0.000 \\
\midrule
Overall & 20 & 8.30 & 9.15 & 0.242 \\
\bottomrule
\end{tabular}
\end{table}
...
All four self-confidence items improved (Table~\ref{tab:confidence}), with the largest gain in ``analyzing test failures'' ($+0.40$, 7~improved, 0~declined).
Internal consistency was good (Cronbach $\alpha_{pre} = 0.811$, $\alpha_{post} = 0.839$).

\begin{table}[htbp]
\caption{Self-Confidence Shift ($N = 20$)}
...
\midrule
Understand concepts & 3.45 & 3.60 & +0.15 & 3/15/2 \\
Write test case & 3.50 & 3.75 & +0.25 & 5/14/1 \\
Identify/debug errors & 3.50 & 3.70 & +0.20 & 4/14/2 \\
Analyze test failures & 3.25 & 3.65 & +0.40 & 7/13/0 \\
\bottomrule
\end{tabular}
\end{table}
...
The game recorded 3,226~events from 25~active players (Table~\ref{tab:telemetry}).
Students with questionnaire matches averaged 46~minutes of active gameplay.

\begin{table}[htbp]
\caption{Sojourner Telemetry: Event Distribution ($N = 25$ players)}
\label{tab:telemetry}
\centering
\small
...
The non-significant MCQ improvement ($p = 0.104$) should not be interpreted as ineffectiveness.
...
The replication package is available at: \url{https://github.com/aazaadef/sojourner-fandango-replication}
