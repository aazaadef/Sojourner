...
\begin{abstract}
Software testing and debugging matter a lot in software engineering, but students often see them as boring, and it can be hard for them to turn theory into real skills. To make the experience more practical, we ran a classroom study with \emph{Sojourner under Sabotage}, a browser-based serious game where students write JUnit tests, build up line coverage, spot mutations, and debug sabotaged spaceship components through seven levels that get harder as they progress. We set up a self-hosted version of the game for 22 master's students. During the study, we collected pre- and post-session multiple-choice quizzes (11 questions), confidence ratings, post-game surveys, open-ended feedback, and detailed in-game telemetry: 3,161 events across 14 event types. The study found that multiple-choice quiz scores went up from 8.29 to 9.14 out of 11, which represents a medium effect size ($r = 0.349$). The biggest improvement was seen among students who had lower initial scores (normalized gain = 0.50). More students became confident in their test analysis skills ($+0.38$, 7~improved, 0~declined). Also, around all students (95\%) agreed that the activity improved their understanding of the difference between testing and debugging, and 86\% found it engaging. In addition, telemetry showed that those who progressed further in the game improved more on their quizzes, with MCQ gains correlating significantly with game progression (Spearman $\rho = 0.549$, $p = 0.006$). This demonstrates a deep connection between progress and learning outcomes. This paper demonstrates how game-based training can enhance software testing skills, but also highlights the ceiling effect, a key lesson of the method. We also show that structured learning helps promote equity in education.


% Results show that MCQ scores improved from 8.29 to 9.14/11 ($r = 0.349$, medium effect), with low-baseline students benefiting most (normalized gain = 0.50). Self-confidence in analyzing test failures increased ($+0.38$, 7~improved, 0~declined), 95\% of students agreed the activity helped them understand testing vs.\ debugging, and 86\% found it engaging. Telemetry further corroborated these findings: game progression correlated significantly with MCQ gains (Spearman $\rho = 0.549$, $p = 0.006$), confirming that deeper engagement produces greater learning. We discuss implications for game-based software testing education, the ceiling effect as a methodological lesson, and the equity benefits of scaffolded learning environments.
\end{abstract}
...
\label{sec:intro}
% ===========================================================================
In software engineering, testing and debugging skills are crucial, but students often find them boring and tedious compared to coding ~\cite{straubinger2023survey, garousi2020testing}. The traditional lecture-based teaching has a hard time keeping their attention and creates a large gap between theoretical understanding and real-world application in tests ~\cite{mccauley2008debugging, jamil2016testing}. For this reason, software game-based teaching can recreate test tasks in fun, narrative-driven gameplay and be more effective than traditional teaching with immediate and tangible feedback ~\cite{connolly2012systematic, blanco2023gamification}.
...
Third, we build on previous work by using telemetry data to analyze how students behaved during the game. This gave us more objective evidence of the link between gameplay progression and learning gains---a perspective that was missing from earlier evaluations of this game.
...
For supporting post-session analysis of student behavior, a dedicated web-based reporting dashboard---\emph{UBI-Testing and Sabotage Reports}---was developed by the research team, designed specifically for this study.
...
It provides instructors with the ability to select individual students and view their testing and debugging actions in a structured, readable format without requiring manual inspection of raw JSON files.
...
Together, the custom reporting dashboard and the server-level monitoring infrastructure allowed the research team to observe student engagement patterns in real time and conduct the detailed telemetry-based analysis reported in this study. This combination represents a practical, replicable model for deploying serious games at classroom scale with integrated learning analytics support.

\subsection{Session Structure}
The session lasted approximately 100 minutes, of which 45 minutes were an introductory lecture on software testing concepts, and the next approximately 45–50 minutes were devoted to guided gameplay, with 5–10 minutes allocated to completing pre- and post-activity questionnaires.

% The session took approximately 100~minutes: 45~minutes of introductory lecture on software testing concepts, 45--50~minutes of guided gameplay, and 5--10~minutes for pre and post-activity questionnaire completion.
...
The game recorded 3,161~events from 22~active players (Table~\ref{tab:telemetry}).
...
\midrule
Room unlocked & Player moved to a new level & 1,297 \\
Game progression & Room phase changed (e.g., talk $\to$ test $\to$ debug) & 420 \\
Conversation & Dialogue with robot companion completed & 319 \\
Test modified & Student edited test code in the editor & 276 \\
Test executed & Tests compiled and ran successfully & 219 \\
Test exec.\ failed & Compilation error or test assertion failed & 113 \\
Component fixed & Student correctly repaired sabotaged code & 106 \\
CUT modified & Student edited the class-under-test during debugging & 89 \\
Mutation detected & Student's tests caught the injected fault & 68 \\
Tests activated & Tests reached $\geq$50\% coverage and were set to monitor & 61 \\
Debug started & Sabotage detected; student entered debugging mode & 58 \\
Component destroyed & Sabotage went undetected; component damaged & 56 \\
Game started & Player logged in and began a session & 43 \\
Hidden test added & System provided a hint test after undetected sabotage & 36 \\
\midrule
\textbf{Total} & & \textbf{3,161} \\
\bottomrule
\end{tabular}
\end{table}
...
An exploratory correlation analysis ($N = 21$ students with matched telemetry and questionnaire data) revealed a significant positive association between game progression and MCQ gains (Spearman $\rho = 0.549$, $p = 0.006$; Fig.~\ref{fig:correlation}).
Progression was also strongly correlated with test executions ($\rho = 0.912$, $p < 0.001$), confirming that advancement reflects genuine testing activity.

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figures/fig_correlation.pdf}
\caption{(a) Game progression vs.\ MCQ gains ($\rho = 0.549$, $p = 0.006$). (b) Progression vs.\ test executions ($\rho = 0.912$, $p < 0.001$).}
...
The significant correlation between progression and MCQ gains ($\rho = 0.549$, $p = 0.006$) provides evidence that the game's design---where advancement requires genuine testing and debugging skill---translates behavioral engagement into measurable learning.
The 36~hidden-test scaffolding events confirm that the game's adaptive support mechanism actively assisted struggling students, and the strong correlation between progression and test executions ($\rho = 0.912$) rules out passive navigation as an explanation.
The telemetry-based analysis demonstrates the practical advantage of game-based platforms over traditional exercises, as they are able to automatically record and analyze learning data, allowing for ongoing assessment of learning without the need for additional tools.
...
Students repeatedly cited the interactive aspects of the game as one of its strengths, especially the immediate feedback they received after testing and debugging. Many also stated that observing errors occur and subsequently fixing broken components themselves helped them better understand the difference between testing and debugging. This is a distinction that is not always clear in a typical lecture-based teaching. On the other hand, some students had some difficulty with the Java syntax and the complexity of debugging, suggesting that beginners need more guidance when starting to use such software games. In short, these qualitative insights are in line with the quantitative results and show how the learning process was experienced by students and why they found such activities engaging and educationally valuable.


% Students often pointed to the interactive parts of the game as one of its strengths, especially the immediate feedback they received after testing and debugging actions. Many also said that seeing failures happen, and then repairing the sabotaged components themselves, helped them better understand the difference between testing and debugging. This is a distinction that does not always come across clearly in a regular lecture. At the same time, some students struggled with Java syntax and the complexity of debugging, which suggests that beginners still need extra guidance when they start using the game. Overall, these qualitative insights line up with the quantitative results by showing how students experienced the learning process and why they found the activity engaging and educationally valuable.

\subsection{Equity Through Scaffolding}
Rather than providing uniform improvement for all students, this learning environment showed that students with lower baselines benefited the most (normalized benefit = 0.50 vs. 0.00 for higher baselines), indicating that the game’s scaffolding mechanisms, including robot hints, hidden tests, and progressive difficulty, provided support tailored to need and suggesting that this approach had a compensatory or equity-enhancing effect. This feature is particularly important for heterogeneous classrooms where students enter with different levels of prior knowledge.
...
The cognitive load items mix positive and negative valence (one item is reverse-coded), and the resulting scale showed poor internal consistency($\alpha<0$), suggesting that these items may not reliably measure a single construct.
Self-reported perception may not fully reflect actual learning. Cognitive load findings should be interpreted descriptively rather than as a validated construct score.
...
\textbf{Reliability.}
The sample size ($N = 22$) limits statistical power, and the correlation analysis ($N = 21$ matched students) is exploratory.
...
Behavioral telemetry from 3,161 events further supported these findings, revealing a significant correlation between game progression and learning gains ($\rho = 0.549$, $p = 0.006$).
...
The replication package is available at: \url{https://github.com/aazaadef/Sojourner}
